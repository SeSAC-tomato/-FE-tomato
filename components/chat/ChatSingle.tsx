...
const ChatSingle = ({ chat, isMine }: Props) => {
  ...
  return (
    <>
      ...
      {/*{chat.chatType == ChatType.EVENT_BOOK && (*/}
      {/*  <ChatSingleEventBook*/}
      {/*    chat={chat}*/}
      {/*    chatType={chat.chatType}*/}
      {/*    isMine={isMine}*/}
      ...
      {/*  />*/}
      {/*)}*/}
      {/*{chat.chatType == ChatType.EVENT_END && (*/}
      {/*  <ChatSingleEventEnd*/}
      {/*    chat={chat}*/}
      {/*    chatType={chat.chatType}*/}
      {/*    isMine={isMine}*/}
  ...
  return (
    <>
      {isMine &&  (
        ...
        >
          {chat.chatType != ChatType.MESSAGE && <ChatPostInfo post={chat.post} /> }
          <p className="m-0 whitespace-pre-wrap">
            ...
            {chatType != ChatType.MESSAGE && <span className="font-semibold">{decodeHtml(chat.content)}</span>}
      ...
      {isMine && chatType == ChatType.IMAGE && (
        ...
        >
          {chat.images && chat.images.length > 0 && (
            <a
              href={'http://localhost:8080/api/v1/tes/images/' + chat.images[0]}
              target="_blank"
              rel="noopener noreferrer"
            >
              <img
                src={
                  'http://localhost:8080/api/v1/tes/images/' + chat.images[0]
                }
                alt="alt"
              />
            </a>
...
// type ChatSingleEventBookProps = {
//   chat: ChatResponse;
//   isMine: boolean;
//   chatType: ChatType.EVENT_BOOK;
// };
//
// const ChatSingleEventBook = ({
//   chat,
//   isMine,
//   chatType,
// }: ChatSingleEventBookProps) => {
//   return (
//     <>
//       {isMine && chatType == ChatType.EVENT_BOOK && chat.isEventDone && (
//         <div>1-1{/* 요청했고 수락함 */}</div>
//       )}
//       {isMine && chatType == ChatType.EVENT_BOOK && !chat.isEventDone && (
//         <div>1-2{/* 요청했고 아직 수락안함 */}</div>
//       )}
//       {!isMine && chatType == ChatType.EVENT_BOOK && chat.isEventDone && (
//         <div>1-3{/* 요청받았고 수락함 */}</div>
//       )}
//       {!isMine && chatType == ChatType.EVENT_BOOK && !chat.isEventDone && (
//         <div>1-4{/* 요청받았고 아직 수락안함 */}</div>
//       )}
//     </>
//   );
// };
//
// type ChatSingleEventEndProps = {
//   chat: ChatResponse;
//   isMine: boolean;
//   chatType: ChatType.EVENT_END;
// };
//
// const ChatSingleEventEnd = ({
//   chat,
//   isMine,
//   chatType,
// }: ChatSingleEventEndProps) => {
//   return (
//     <>
//       {isMine && chatType == ChatType.EVENT_END && chat.isEventDone && (
//         <div>2-1{/* 요청했고 수락함 */}</div>
//       )}
//       {isMine && chatType == ChatType.EVENT_END && !chat.isEventDone && (
//         <div>2-2{/* 요청했고 아직 수락안함 */}</div>
//       )}
//       {!isMine && chatType == ChatType.EVENT_END && chat.isEventDone && (
//         <div>2-3{/* 요청받았고 수락함 */}</div>
//       )}
//       {!isMine && chatType == ChatType.EVENT_END && !chat.isEventDone && (
//         <div>2-4{/* 요청받았고 아직 수락안함 */}</div>
//       )}
//     </>
//   );
// };
